            ...
            for (const ref of elementReferences) {
                ...
                for (const attr of node.attrs || []) {
                  ...
                  i.e. you must write:  ${attr.name}$="${attr.value}"`),
                    ...
                    const allowedBindings = isAttribute ? element.attributes : element.properties;
...
function closestOption(name, isAttribute, element) {
    const attributeOptions = element.attributes.map((a) => a.name)
        .concat(Array.from(util_1.sharedAttributes.keys()));
    const propertyOptions = element.properties.map((a) => a.name)
        .concat(Array.from(util_1.sharedProperties.keys()));
