...
suite('PolymerCoreFeatureScanner', () => {
    ...
    `;
        ...
        chai_1.assert.deepEqual(a.properties.map(({ name, type, description }) => ({ name, type, description })), [{
                name: 'propA',
                type: 'Array',
                description: 'Prop A',
            }]);
        chai_1.assert.deepEqual(a.methods.map(({ name, type, description }) => ({ name, type, description })), [{
                name: 'methodA',
                type: 'Function',
        ...
        chai_1.assert.deepEqual(b.properties, []);
        chai_1.assert.deepEqual(b.methods.map(({ name, type, description }) => ({ name, type, description })), [{
                name: 'methodB',
                type: 'Function',
                description: '',
            }]);
        ...
        chai_1.assert.deepEqual(base.properties, []);
        chai_1.assert.deepEqual(base.methods.map(({ name, type, description }) => ({ name, type, description })), [{
                name: 'methodBase',
                type: 'Function',
                description: '',
            }]);
    ...
    test('resolves the Polymer.Base class', () => __awaiter(this, void 0, void 0, function* () {
        ...
        chai_1.assert.lengthOf(polymerBase.methods, 35);
        chai_1.assert.lengthOf(polymerBase.properties, 2);
        ...
        const addFeature = polymerBase.methods.find((m) => m.name === '_addFeature');
        chai_1.assert.isDefined(addFeature);
        chai_1.assert.equal(addFeature.privacy, 'protected');
        // A property from behaviors.html
        const behaviors = polymerBase.properties.find((m) => m.name === 'behaviors');
