...
function serializeClass(class_, packagePath) {
    ...
    const properties = class_.properties.map((p) => serializeProperty(class_, path, p));
...
function serializeElementLike(elementOrMixin, packagePath) {
    ...
    class_.attributes = elementOrMixin.attributes.map((a) => serializeAttribute(elementOrMixin, path, a));
    class_.events =
