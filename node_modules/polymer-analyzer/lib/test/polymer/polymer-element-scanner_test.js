    ...
    suite('scan()', () => {
      ...
      Polymer({
        ...
        listeners: []
            ...
            chai_1.assert.equal(features[0].properties.length, 9);
            chai_1.assert.deepEqual(features[0]
            ...
            chai_1.assert.deepEqual(features[0].methods.map((m) => m.name), [
                'customPublicMethod',
            ...
            chai_1.assert.deepEqual(features[0].attributes.map((p) => [p.name, p.changeEvent]), [
                ['a', undefined],
                ['b', undefined],
                ['c', undefined],
                ['d', undefined],
      ...
      Polymer({
        ...
        method() {
          const shouldIgnore = {
            ...
            chai_1.assert.deepEqual(element.properties.map((p) => p.name), ['parseError', 'badKindOfExpression']);
            chai_1.assert.deepEqual(yield Promise.all(element.properties.map((p) => underliner.underline(p.warnings))), [
                [
                    `
            computed: 'let let let',
                           ~`,
